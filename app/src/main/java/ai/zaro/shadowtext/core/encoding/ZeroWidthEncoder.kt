...
class ZeroWidthEncoder : InvisibleEncoder {
    ...
    override fun decode(encoded: String): ByteArray {
        ...
        if (invisible.isEmpty() || invisible.length < 8) {
            throw EncodingException("No encoded data found in text")
        }
    ...
    override fun containsEncodedData(text: String): Boolean {
        ...
        return invisible.length >= 8 && invisible.contains(String(START_MARKER)) && invisible.contains(String(END_MARKER))
    }

    override fun encodedCharCount(byteCount: Int): Int = byteCount * 4 + 8

    private fun bitsToChar(bits: Int): Char = when (bits and 0x03) { 0 -> ZWSP; 1 -> ZWNJ; 2 -> ZWJ; else -> BOM }

    private fun charToBits(ch: Char): Int = when (ch) { ZWSP -> 0; ZWNJ -> 1; ZWJ -> 2; BOM -> 3; else -> throw EncodingException("Unexpected char: U+%04X".format(ch.code)) }
