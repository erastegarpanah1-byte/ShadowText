...
@Composable
fun DecodeInputScreen(onBack: () -> Unit, onDecode: (String) -> Unit) {
    ...
    Scaffold(containerColor = c.background, topBar = { TopAppBar(title = { Text(stringResource(R.string.decode_title), fontWeight = FontWeight.SemiBold, color = c.onBackground) }, navigationIcon = { IconButton(onClick = onBack) { Icon(Icons.AutoMirrored.Filled.ArrowBack, stringResource(R.string.back), tint = c.onBackground) } }, colors = TopAppBarDefaults.topAppBarColors(containerColor = c.background)) }) { padding ->
        ConstrainedColumn(modifier = Modifier.padding(padding)) {
            ...
            Button(onClick = { isLoading = true; onDecode(inputText) }, enabled = inputText.isNotBlank() && !isLoading, modifier = Modifier.fillMaxWidth().height(52.dp), shape = RoundedCornerShape(14.dp), colors = ButtonDefaults.buttonColors(containerColor = c.secondary, contentColor = c.onSecondary)) {
                if (isLoading) { CircularProgressIndicator(Modifier.size(20.dp), color = c.onSecondary, strokeWidth = 2.dp); Spacer(Modifier.width(8.dp)) }
